...
Ro_2 = Constant(10**3) # denominator of grad(theta)
...
T_ = Function(V_2).interpolate(1 + bell_1)
...
F = ( inner(u-u_,v)
    ...
    - Dt * inner(f, v) 
    + (T -T_)*phi 
    + Dt*half*(inner(u_,grad(T_)) + inner(u,grad(T)))*phi # removed the diffusion term
