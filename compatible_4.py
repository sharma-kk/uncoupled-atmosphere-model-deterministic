...
Ro_1 = Constant(10**4) # denominator of perp(u)
Ro_2 = Constant(10**2) # denominator of grad(theta)
...
T_ = Function(V0).interpolate(1 + bell_1)
...
outfile = File("./results/c4.pvd")
u_.rename("atm_vel")
# To.rename("ocean_temp")
outfile.write(u_, project(T_, V0_out, name="atm_temp"))
...
while (round(t,4) <= t_end):
    ...
    if iter_n%freq == 0:
        ...
        print("t=", round(t,4))
        u.rename("atm_vel")
        outfile.write(u, project(T, V0_out, name="atm_temp"))
    u_.assign(u)
    T_.assign(T)
